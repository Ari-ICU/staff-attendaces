...
export default function PayrollDetails({ report, onClose }: PayrollDetailsProps) {
  ...
  const notes = "Payment processed via direct deposit.";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm transition-all duration-300">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl max-w-lg w-full mx-4 p-6 sm:p-8 transform transition-all duration-300 scale-100 hover:scale-[1.01]">
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">Employee ID: {report.id}</p>
            </div>
          </div>
        ...
        <div className="mt-8 flex justify-end gap-4">
          <button
            onClick={onClose}
            className="px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors duration-200"
          >
            Close
          </button>
          <button
            className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition-colors duration-200"
            onClick={() => alert("Download functionality not implemented")} // Placeholder action
          >
            Download Receipt
          </button>
        </div>
